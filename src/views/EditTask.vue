  ...
  <div>
    ...
    <main>
      ...
      <div class="create__task">
        <input type="text" placeholder="Please Enter Your Title" v-model="task[0].title">
        <textarea cols="30" rows="10" placeholder="Please Enter Your text" v-model="task[0].body"></textarea>
      </div>
    </main>
    <footer>
...
export default {
  ...
  methods:{
    updateTask(){
      let findTask = this.allTasks.find(item=> item.id === this.task[0].id)
      let objId= this.allTasks.indexOf(findTask);
      this.allTasks[objId].title = this.task[0].title;
      this.allTasks[objId].body = this.task[0].body;
  ...
  mounted() {
    ...
    this.task = this.allTasks.filter(items => items.id == this.categoryId &&  items.status === 'pending')
  }
}
</script>
